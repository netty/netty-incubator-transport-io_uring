...
abstract class AbstractIOUringServerChannel extends AbstractIOUringChannel implements ServerChannel {
    ...
    protected AbstractIOUringServerChannel(EventLoop eventLoop, EventLoopGroup childEventLoopGroup, int fd) {
        this(eventLoop, childEventLoopGroup, new LinuxSocket(fd), false);
    }

    AbstractIOUringServerChannel(EventLoop eventLoop, EventLoopGroup childEventLoopGroup, LinuxSocket fd) {
        this(eventLoop, childEventLoopGroup, fd, isSoErrorZero(fd));
    }

    AbstractIOUringServerChannel(EventLoop eventLoop, EventLoopGroup childEventLoopGroup,
        ...
        acceptedAddressMemoryAddress = Buffer.nativeAddressOf(acceptedAddressMemory);
        ...
        acceptedAddressLengthMemoryAddress = Buffer.nativeAddressOf(acceptedAddressLengthMemory);
    }
    ...
    abstract Channel newChildChannel(int fd, long acceptedAddressMemoryAddress, long acceptedAddressLengthMemoryAddress) throws Exception;
    ...
    final class UringServerChannelUnsafe extends AbstractIOUringChannel.AbstractUringUnsafe {
        ...
        protected void readComplete0(int res, int data, int outstanding) {
            final IOUringRecvByteAllocatorHandle allocHandle = (IOUringRecvByteAllocatorHandle) unsafe()
                    .recvBufAllocHandle();
