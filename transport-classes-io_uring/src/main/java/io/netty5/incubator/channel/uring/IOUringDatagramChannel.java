...
import static io.netty5.channel.socket.InternetProtocolFamily.IPv6;
import static io.netty5.channel.unix.Errors.ioResult;
import static io.netty5.incubator.channel.uring.LinuxSocket.newSocketDgram;
...
public final class IOUringDatagramChannel extends AbstractIOUringChannel implements DatagramChannel {
     ...
     * on the Operation Systems default which will be chosen.
     */
    public IOUringDatagramChannel(EventLoop eventLoop, InternetProtocolFamily family) {
        this(eventLoop, newSocketDgram(family == IPv6), false);
    }
    ...
    final class IOUringDatagramChannelUnsafe extends AbstractUringUnsafe {
        ...
        protected void readComplete0(int res, int data, int outstanding) {
            ...
            try {
                if (data == -1) {
                    ...
                    readComplete(pipeline, allocHandle, buffer, res);
                } else {
                    recvmsgComplete(pipeline, allocHandle, buffer, res, data, outstanding);
                }
            } catch (Throwable t) {
                if (connected && t instanceof Errors.NativeIoException) {
                    t = translateForConnected((Errors.NativeIoException) t);
                }
        ...
        private void readComplete(ChannelPipeline pipeline, IOUringRecvByteAllocatorHandle allocHandle,
                                  Buffer buffer, int res) throws IOException {
            try {
                ...
                pipeline.fireChannelRead(new DatagramPacket(buffer, IOUringDatagramChannel.this.localAddress(),
        ...
        private void recvmsgComplete(ChannelPipeline pipeline, IOUringRecvByteAllocatorHandle allocHandle,
                                      Buffer buffer, int res, int idx, int outstanding) throws IOException {
            ...
            if (res < 0) {
                ...
                allocHandle.lastBytesRead(res);
                allocHandle.incMessagesRead(1);
                DatagramPacket packet = hdr.read(IOUringDatagramChannel.this, buffer, res);
        ...
        protected int scheduleRead0() {
            ...
            int numDatagram = datagramSize == 0 ? 1 : Math.max(1, buffer.writableBytes() / datagramSize);

            if (isConnected() && numDatagram <= 1) {
                submissionQueue().addRead(socket.intValue(), buffer.forEachReadable().first().readableNativeAddress(),
                        buffer.writerOffset(), buffer.capacity(), (short) -1);
                return 1;
            } else {
                int scheduled = scheduleRecvmsg(buffer, numDatagram, datagramSize);
                if (scheduled == 0) {
        ...
        private int scheduleRecvmsg(Buffer buffer, int numDatagram, int datagramSize) {
            int writable = buffer.writableBytes();
            IOUringSubmissionQueue submissionQueue = submissionQueue();
            long bufferAddress = buffer.forEachReadable().first().readableNativeAddress() + buffer.writerOffset();
            if (numDatagram <= 1) {
        ...
        private boolean scheduleRecvmsg0(IOUringSubmissionQueue submissionQueue, long bufferAddress,
                                         int bufferLength) {
    ...
    /**
     * Returns {@code true} if the usage of {@link SegmentedDatagramPacket} is supported.
     *
