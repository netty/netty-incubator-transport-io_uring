...
final class MsgHdrMemory {
    ...
    DatagramPacket read(IOUringDatagramChannel channel, Buffer buffer, int bytesRead) {
        ...
        if (channel.socket.isIpv6()) {
            byte[] ipv6Bytes = channel.registration.ioUringHandler().inet6AddressArray();
            byte[] ipv4bytes = channel.registration.ioUringHandler().inet4AddressArray();

            sender = SockaddrIn.readIPv6(sockAddress, ipv6Bytes, ipv4bytes);
        } else {
            byte[] bytes = channel.registration.ioUringHandler().inet4AddressArray();
        ...
        // in the iovec.
        long memoryAddress = buffer.forEachReadable().first().readableNativeAddress();
        int readerIndex = (int) (bufferAddress - memoryAddress);

        Buffer slice = buffer.readerOffset(readerIndex)
                .readSplit(bufferLength)
                .writerOffset(bytesRead);
        return new DatagramPacket(slice, channel.localAddress(), sender);
    }
